...
void MoveToCrateRotateEncoderTarget(int nEncoderTarget)
{
  nMotorEncoderTarget[LeftCrateRotate]  = nEncoderTarget;
  nMotorEncoderTarget[RightCrateRotate] = nEncoderTarget;

  int nNxtMotorPower = (nEncoderTarget > 0) ? CRATE_ROTATE_POWER : -CRATE_ROTATE_POWER;
...
void SetCrateRotationAction(TCrateRotate rot)
{
  ...
  // MR T ... I'M BETTING THIS NEEDS TO MOVE TO ITS OWN TASK SO THE runStateIdle CHECK CAN RELIABLY WORK
  //      ... I'VE SEEN THIS TIME SLICE APPROACH MISS THE IDLE STATE INDICATION MORE OFTEN THAN NOT
  ...
  {
      MoveToCrateRotateEncoderTarget(CRATE_ROTATE_DEFAULT_TURN);
      bCrateRotating = true;
  }
  ...
  {
      MoveToCrateRotateEncoderTarget(CRATE_ROTATE_SMALL_TURN);
      bCrateRotating = true;
  }
...
TCrateLiftDestination CurrCrateLiftDest()
{
...
long CurrCrateLiftEncoder()
{
  return nMotorEncoder[CrateLift];
}

void SetCrateLiftEncoderTarget(long tgt)
{
	  nMotorEncoderTarget[CrateLift] = tgt;
}

TNxtRunState CrateLiftMotorRunState()
{
...
void MoveToCrateLiftDestination(TCrateLiftDestination crateLiftDest)
{
  ...
  long iCurrLiftLoc = CurrMotorEncoder();
  ...
  else
  {
	  ...
	  // Set the target
	  SetCrateLiftEncoderTarget(iRelativeMove);
...
void SetCrateLiftDestination(TCrateLiftDestination crateLiftDest)
{
  ...
  else if ( CrateLiftIsMoving() == false )
  {
    ...
    while ( CrateLiftMotorRunState() != runStateIdle )
    {}
    ...
    if ( CurrCrateLiftDest() == CRATE_LIFT_TO_GRAB )
      _tCrateLiftState = CRATE_LIFT_AT_GRAB;
    else if ( CurrCrateLiftDest() == CRATE_LIFT_TO_BALL )
      _tCrateLiftState = CRATE_LIFT_AT_BALL;
    else if ( CurrCrateLiftDest() == CRATE_LIFT_TO_ROTATE )
      _tCrateLiftState = CRATE_LIFT_AT_ROTATE;
    else if ( CurrCrateLiftDest() == CRATE_LIFT_TO_CHIMNEY )
  ...
  {
	  if ( CrateLiftMotorRunState() == runStateIdle )
	  {
	    SetCrateLiftMotorPower(0);
	    nxtDisplayTextLine(5, "HIT TGT");
...
int DetermineLiftPowerForDest(long nEncoderTarget, TCrateLiftState tTargetState)
{
  int pwr = 0;
  long nCurrLiftEncoder = CurrCrateLiftEncoder();
...
void SetCrateLiftDestination(TCrateLiftDestination crateLiftDest)
{
  ...
  {
    nCrateLiftPower = DetermineLiftPowerForDest(CRATE_LIFT_GRAB_ENCODER, CRATE_LIFT_AT_GRAB);
  }

  // go to ball load position?
  else if ( crateLiftDest == CRATE_LIFT_TO_BALL )
  {
    nCrateLiftPower = DetermineLiftPowerForDest(CRATE_LIFT_BALL_ENCODER, CRATE_LIFT_AT_BALL);
  }

  // go to crate rotate position
  else if ( crateLiftDest == CRATE_LIFT_TO_ROTATE )
  {
    nCrateLiftPower = DetermineLiftPowerForDest(CRATE_LIFT_ROTATE_ENCODER, CRATE_LIFT_AT_ROTATE);
  }

  // go to chimney load position
  else if ( crateLiftDest == CRATE_LIFT_TO_CHIMNEY )
  {
    nCrateLiftPower = DetermineLiftPowerForDest(CRATE_LIFT_CHIMNEY_ENCODER, CRATE_LIFT_AT_CHIMNEY);
  }
...
void DisplayCrateDiags()
{
  float fCurrEncoder = CurrCrateLiftEncoder();
  ...
  string idle = (CrateLiftMotorRunState() == runStateIdle) ? "yes" : "no";
...
//
//    Button presses are ignored while releasing a ball
//
