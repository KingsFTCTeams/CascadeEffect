...
task updateArm() {
	setMotorSpeedPID(turnMotor1, calcPosPID(turnMotor1, turnMotor1.currentGoalSpeedDegreesPerSecond, turnMotor1.currentGoalPosDegrees));
}

void initArm() {
	initPIDMotor(turnMotor1, ARM_MOTOR_ONE, DEFAULT_POS_KP, DEFAULT_POS_KI, DEFAULT_POS_KD,
...
int calcPosPID(pidMotor *pid, int goalDegreesPerSecond, int goalPosDegrees) {
	...
	pid->goalPosTicks = goalPosDegrees * 4.2;
	...
	if(pid->posOutput > goalDegreesPerSecond && pid->posOutput > 0) pid->posOutput = goalDegreesPerSecond;
	else if(pid->posOutput < -goalDegreesPerSecond && pid->posOutput < 0) pid->posOutput = -goalDegreesPerSecond;
