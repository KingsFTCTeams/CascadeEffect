...
#define WithinRange(val1, val2, range)  ( abs(val1 - val2) <= range )
...
#define joy1TopHat_PRESSED (joystick.joy1_TopHat != -1)
...
#define joy2TopHat_PRESSED (joystick.joy2_TopHat != -1)
...
int ScaleJoystickInput(float input)
{
  ...
  input = (input < 0)? input - 0.5 : input + 0.5;
